...
final class SuspectSet {
  ...
  /// Only the WIDGET-LIFECYCLE globs (`*State`, `*Screen`) are [appOnly]: the
  /// framework spawns and disposes hundreds of private `*State` objects
  /// (`_FocusState`, `_AnimatedState`, `NavigatorState`, …) as you navigate, and
  /// flagging that churn floods the report — so those only flag app-owned
  /// matches.
  ///
  /// The DISPOSABLE-RESOURCE globs (`*Controller`, `*Notifier`, `*Stream*`,
  /// `*Timer`, `*Bloc`, `*Cubit`) are NOT app-filtered: a growing
  /// `TextEditingController` / `AnimationController` / `_Timer` /
  /// `StreamController` is a real, useful leak wherever its class is declared,
  /// so platform resource leaks surface. The growth metric (latest minus the
  /// window minimum) keeps healthy churn — which returns to baseline — out.
  ///
  /// Explicit user rules added via [LeakRadarConfig.rules] are never
  /// app-filtered (they default `appOnly: false`).
