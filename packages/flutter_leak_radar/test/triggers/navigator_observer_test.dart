...
void main() {
  group('LeakRadarNavigatorObserver', () {
    ...
    testWidgets('single didPop triggers exactly one scan after debounce', (
      tester,
    ) async {
      var scanCount = 0;
    ...
    testWidgets('rapid didPop calls are coalesced into a single scan', (
      tester,
    ) async {
      var scanCount = 0;
      final observer = LeakRadarNavigatorObserver(
    ...
    testWidgets('didPush and didReplace do not trigger a scan', (tester) async {
      ...
      observer.didReplace(newRoute: _FakeRoute(), oldRoute: _FakeRoute());
    ...
    testWidgets('dispose cancels pending timer — no scan fires after dispose', (
      tester,
    ) async {
      var scanCount = 0;
      final observer = LeakRadarNavigatorObserver(
