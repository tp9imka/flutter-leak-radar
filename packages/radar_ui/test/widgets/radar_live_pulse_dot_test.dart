  ...
  group('RadarLivePulseDot', () {
    testWidgets('renders a dot', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: Scaffold(body: RadarLivePulseDot())),
      );
      ...
      (tester) async {
        await tester.pumpWidget(
          const MediaQuery(
            ...
            child: MaterialApp(home: Scaffold(body: RadarLivePulseDot())),
          ),
        );
        // Widget must render without error under reduced-motion.
        expect(find.byType(RadarLivePulseDot), findsOneWidget);
        // No FadeTransition should be rendered inside the dot when animations
        // are disabled.
        expect(
          find.descendant(
            of: find.byType(RadarLivePulseDot),
